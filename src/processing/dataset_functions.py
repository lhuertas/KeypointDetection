...
class DatasetTransformer:
    ...
    def init_grid(self):
        ...
        yy, xx = tf.meshgrid(y_grid, x_grid, indexing='ij')  # indexing is a must
    ...
    def keypoints_layer(self, kpts_layer):
        ...
        and this reduces them to a single array by the minimum of the distances.
    ...
    def single_PAF(self, joint):
        ...
        *does not support batched input
        ...
        if joint[4] == tf.constant(0.0) or tf.reduce_all(jpts[1] - jpts[0] == 0.0):
            ...
            bounded_normal_gaussian_mag = normal_gaussian_mag * limit  # bound the normal distance by the endpoints TODO: review if boolean

            max_magnitude = tf.math.reduce_max((begin_gaussian_mag, end_gaussian_mag, bounded_normal_gaussian_mag), axis=0)

            vector_mag = tf.stack((max_magnitude, max_magnitude), axis=-1) # TODO: debug
