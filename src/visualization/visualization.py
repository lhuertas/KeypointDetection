...
def draw_skeleton(img, keypoints, keypoints_names=None, figure_size=8):
    plt.figure(figsize=(figure_size, figure_size))
    keypoints_names = list(range(23))
    for i,coord in enumerate(keypoints):
        kpt_coord = (coord[0],coord[1])
        cv2.circle(img, kpt_coord, draw_config.keypoint_circle_diameter, draw_config.keypoint_circle_color)
        cv2.putText(img=img
                    , text=str(keypoints_names[i])
                    , org=kpt_coord
                    , fontFace=cv2.FONT_HERSHEY_SIMPLEX
                    , fontScale=draw_config.keypoint_text_scale
                    , thickness=draw_config.keypoint_text_thickness
                    , color=draw_config.keypoint_text_color)
    plt.imshow(img)
    plt.show()
...
class SkeletonDrawer:
    ...
    def kpt_draw(self, kpt_coord, kpt_name):
        ...
        self.img = cv2.circle(self.img, kpt_coord, self.dc.keypoint_circle_diameter, self.dc.keypoint_circle_color)
